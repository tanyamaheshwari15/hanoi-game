...
import {getHanoiSolutions} from './tower.js'

//select all tower elements
const towers = document.querySelectorAll('.tower')

//initialize tower content as an array of arrays representing the discs on each tower
let towerContent = [[],[],[]]

//initialize the size of the disc
let size=3
...
//colors of the disc
const DISC_COLOR = ['#023b5c' , 'goldenrod' , '#5c0220ee' , '#3f796cee' , '#663f79ee' , '#CC9999'];

//initial width of the disc
const startWidth = 90
...
//variables to track current and origin tower during dragging
let currentTower
let originTower

//functions to build the towers with stems and plates
...
function start(){
    //rest the tower content array
    towerContent=[[],[],[]]

    //build the towers with stems and plates
    buildTowers(towers)

    //check discs and place them on the first tower
    for(let i=0; i< size; i++){
        const tower = document.createElement('div')
        tower.classList.add('disc')
        tower.draggable = true
        tower.style.backgroundColor=DISC_COLOR[i]
    ...
    //add event listerns for drangenter and dragover to each tower
...
function moveTower(originTowerIndex, currentTowerIndex, disc){
    if(isDroppable(originTowerIndex, currentTowerIndex, disc)){
        towerContent[currentTowerIndex].push(towerContent
            [originTowerIndex].pop())
...
//check if disc is on top of the origin tower
...
function isDiscLessThan(currentTowerIndex, disc){
    ...
    if(!towerContent[currentTowerIndex][size-1]){
        ...
        let sizeBottom = towerContent[currentTowerIndex][size -1].style.width.substring(
            0, towerContent[currentTowerIndex][size-1].style.width.indexOf('p'))
...
class Game{
    ...
    newGame = () => {
        ...
        //event listerner for the new button click
